...
// ⬇️ use the AuthContext helper so requests include cookies + Bearer token
...
const AdminDashboard = () => {
  ...
  // from AuthContext; fetchWithAuth adds Authorization and sends cookies
  const { fetchWithAuth, API_URL } = useAuth();
  const ADMIN_BASE = `${(API_URL || "http://127.0.0.1:5000")}/api/admin`;

  // Verify admin session, then load dashboard data
  useEffect(() => {
    const guardAndLoad = async () => {
      const isAuthenticated = localStorage.getItem("adminAuthenticated") === "true";
      const adminUserRaw = localStorage.getItem("authUser") || localStorage.getItem("adminUser");

      if (!isAuthenticated || !adminUserRaw) {
      ...
      // Allow either is_admin boolean or role === 'admin'
      if (!(adminUser?.is_admin === true || adminUser?.role === "admin")) {
    ...
    guardAndLoad();
    // only run on mount / navigate changes
  }, [navigate]);
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      // Stats
      const statsResponse = await fetchWithAuth(`${ADMIN_BASE}/stats`);
      setStats({
        totalUsers: statsResponse?.totalUsers ?? 0,
        activeBuddies: statsResponse?.activeBuddies ?? 0,
        pendingRequests: statsResponse?.pendingRequests ?? 0,
        projectsCompleted: statsResponse?.projectsCompleted ?? 0,
      });

      // Users
      const usersResponse = await fetchWithAuth(`${ADMIN_BASE}/users`);
      const formattedUsers = Array.isArray(usersResponse)
        ? usersResponse.map((u) => ({
            ...
            specialization: u.profile?.specialization || "Not specified",
            status: u.is_active ? "Active" : "Inactive",
            avatar:
              u.profile?.profile_picture ||
              `https://ui-avatars.com/api/?name=${encodeURIComponent(u.username)}&background=6366f1&color=fff`,
      ...
      // Requests
      const requestsResponse = await fetchWithAuth(`${ADMIN_BASE}/requests`);
      const formattedRequests = Array.isArray(requestsResponse)
        ? requestsResponse.map((r) => ({
            id: r.id,
            from: r.initiator?.username || `User ${r.user_id}`,
            fromId: r.user_id,
            to: r.buddy_user?.username || `User ${r.buddy_id}`,
            toId: r.buddy_id,
      ...
      if (err?.status === 401) {
        // not authenticated — clear and bounce to login
        localStorage.removeItem("adminAuthenticated");
        localStorage.removeItem("adminUser");
        localStorage.removeItem("authUser");
        navigate("/adminlogin", { replace: true });
  ...
  const handleLogout = async () => {
    try {
      ...
      await fetchWithAuth(`${ADMIN_BASE}/logout`, { method: "POST" });
    } catch (_) {}
    localStorage.removeItem("adminAuthenticated");
    localStorage.removeItem("adminUser");
    localStorage.removeItem("authUser");
  ...
  const handleDeleteUser = async (userId) => {
    ...
    try {
      await fetchWithAuth(`${ADMIN_BASE}/users/${userId}`, { method: "DELETE" });
      setUsers((prev) => prev.filter((u) => u.id !== userId));
      setStats((prev) => ({ ...prev, totalUsers: Math.max(0, (prev.totalUsers || 0) - 1) }));
    } catch (err) {
  ...
  const handleToggleUserStatus = async (userId, currentStatus) => {
    try {
      ...
      await fetchWithAuth(`${ADMIN_BASE}/users/${userId}/status`, {
        method: "PUT",
  ...
  const handleApproveRequest = async (requestId) => {
    try {
      await fetchWithAuth(`${ADMIN_BASE}/approve-request/${requestId}`, { method: "POST" });
      setRequests((prev) =>
      ...
      setStats((prev) => ({
        ...
        activeBuddies: (prev.activeBuddies || 0) + 1,
      }));
    } catch (err) {
  ...
  const handleRejectRequest = async (requestId) => {
    try {
      await fetchWithAuth(`${ADMIN_BASE}/reject-request/${requestId}`, { method: "POST" });
      setRequests((prev) =>
  ...
  const UserRow = ({ user, onDelete, onToggleStatus }) => (
    <tr className="hover:bg-gray-50 transition-colors duration-200">
      ...
      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{user.specialization}</td>
      <td className="px-6 py-4 whitespace-nowrap">
        <span
        ...
        {[
          ...
          <button
            ...
            className={`flex items-center space-x-3 px-4 py-3 rounded-lg transition-all duration-200 w-full text-left ${
              activeTab === item.id ? "bg-red-50 text-red-600" : "text-gray-600 hover:text-gray-900 hover:bg-gray-100"
            }`}
          >
            ...
            <div className="max-w-7xl mx-auto">
              ...
              {error && (
                <div className="mb-6 p-4 rounded-lg bg-red-50 text-red-600 border border-red-200">
                  {error}
                  <button onClick={() => setError(null)} className="float-right text-red-800 font-bold">
                    ×
                  </button>
                </div>
              )}
                  ...
                  <div className="p-6 rounded-xl bg-white border border-gray-200 shadow-sm">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                      {[
                        ...
                        <button key={index} className="p-4 rounded-lg text-center transition-all duration-200 bg-gray-50 hover:bg-gray-100 text-gray-900">
                        ...
                        <div key={index} className="flex items-center p-3 rounded-lg hover:bg-gray-50 transition-colors duration-200">
                          ...
                          <div className="flex-1">
                            <p className="text-sm font-medium text-gray-900">
                              <span className="capitalize">{request.from}</span> sent a connection request to{" "}
                              <span className="text-red-600">{request.to}</span>
                            </p>
                  ...
                  <div className="overflow-x-auto">
                    <table className="min-w-full divide-y divide-gray-200">
                      ...
                      <tbody className="bg-white divide-y divide-gray-200">
                        {users.map((u) => (
                          <UserRow key={u.id} user={u} onDelete={handleDeleteUser} onToggleStatus={handleToggleUserStatus} />
                        ))}
                      </tbody>
                    </table>
                  </div>
                        ...
                        {requests.length > 0 ? (
                          requests.map((r) => (
                            <tr key={r.id} className="hover:bg-gray-50">
                              <td className="px-6 py-4 text-sm text-gray-900">{r.from}</td>
